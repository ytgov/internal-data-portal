...
async function uploadAndClose() {
  ...
  try {
    const { datasetFile } = await datasetFilesApi.update(
      props.datasetId,
      props.datasetFileId,
      formData
    )
    ...
    snack.notify("Dataset file updated", { color: "success" })
  } catch (error) {
    snack.notify(`Failed to update dataset file ${error}`, { color: "error" })
  } finally {
